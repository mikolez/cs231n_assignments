...
class CaptioningRNN(object):
    ...
    def sample(self, features, max_length=30):
        ...
        N, D = features.shape
        h_i, _ = affine_forward(features, W_proj, b_proj)
        V, M = W_embed.shape
        start = np.ones(N, dtype=np.int32) * self._start
        x_i, _ = word_embedding_forward(start, W_embed)
        c_i = np.zeros_like(h_i)
        for i in range(max_length):
            if self.cell_type == 'rnn':   
                h_i, _ = rnn_step_forward(x_i, h_i, Wx, Wh, b)
            else:
                h_i, c_i, _ = lstm_step_forward(x_i, h_i, c_i, Wx, Wh, b)
            scores = np.dot(h_i, W_vocab) + b_vocab
            captions[:, i] = np.argmax(scores, axis=1)
            x_i, _ = word_embedding_forward(captions[:, i], W_embed)
